...
# Converts any java.lang.CharSequence to a Python string via StringBuilder.toString(),
# avoiding the GetStringChars fast path which is only valid for String.

cdef JNIRef j_StringBuilder = None
cdef jmethodID mid_StringBuilder_init_CharSequence = NULL
cdef jmethodID mid_StringBuilder_toString = NULL


cdef j2p_char_sequence(JNIEnv *j_env, JNIRef j_object):
    env = CQPEnv.wrap(j_env)
    if not j_object:
        raise ValueError("CharSequence cannot be null")

    global j_StringBuilder, mid_StringBuilder_init_CharSequence, mid_StringBuilder_toString
    if not mid_StringBuilder_toString:
        j_StringBuilder = env.FindClass("java.lang.StringBuilder")
        mid_StringBuilder_init_CharSequence = env.GetMethodID(
            j_StringBuilder, "<init>", "(Ljava/lang/CharSequence;)V")
        mid_StringBuilder_toString = env.GetMethodID(
            j_StringBuilder, "toString", "()Ljava/lang/String;")

    cdef jvalue j_args[1]
    j_args[0].l = (<JNIRef?>j_object).obj
    j_builder = env.NewObjectA(j_StringBuilder, mid_StringBuilder_init_CharSequence, j_args)
    j_string = env.CallObjectMethodA(j_builder, mid_StringBuilder_toString, NULL)
    return j2p_string(j_env, j_string)
...
cdef _chaquopy_unwrap_java_chain_arg(arg):
    """unwraps a _ChaquopyJChain wrapper when passed as a Java method argument."""
    value = getattr(arg, "value", None)
    if value is not None and isinstance(value, JavaObject):
        return value
    receiver = getattr(arg, "receiver", None)
    if receiver is not None and isinstance(receiver, JavaObject):
        return receiver
    return arg


cdef _chaquopy_effective_arg_type(arg):
    """returns the Java type used for overload resolution of a chain-wrapped arg."""
    if hasattr(arg, "value") or hasattr(arg, "receiver"):
        return type(_chaquopy_unwrap_java_chain_arg(arg))
    return type(arg)


cdef j2p_by_definition(JNIEnv *j_env, definition, obj):
    """converts a Python value to Java and back by an explicit JNI signature,
    normalizing e.g. boxed/primitive mismatches for overload resolution."""
    cdef JNIEnv *j_env_ptr = j_env
    j_value = p2j(j_env_ptr, definition, obj)
    if isinstance(j_value, JNIRef):
        return j2p(j_env_ptr, j_value)
    return j_value
