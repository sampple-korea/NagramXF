...
cdef _chaquopy_collect_methods(cls, name, bint access_all=False):
    """collects all methods with the given name across the MRO."""
    jms = []
    for c in cls.__mro__:
        if not isinstance(c, JavaClass):
            continue
        reflector = get_reflector(c)
        members = (reflector.getMethodsAll(name) if access_all
                   else reflector.getMethods(name))
        for m in (members or []):
            jms.append(JavaMethod(cls, name, m))
    return jms
